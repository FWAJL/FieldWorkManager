...
class Globalization extends \Library\BL\Core\ApplicationComponent {
    ...
    public function loadResources() {
        $this->resoures_path = __ROOT__ . \Library\Enums\FolderName::AppsFolderName . $this->app->name;
        //Get list of resource files
        $this->_files_common = $this->getFilesList($this->resoures_path . \Library\Enums\FolderName::ResourceCommonFolderName);
        $this->_files_local = $this->getFilesList($this->resoures_path . \Library\Enums\FolderName::ResourceLocalFolderName);
        //For each resource type, load data into the appropriate array
        foreach ($this->_files_common as $file) {
            $this->loadFile("common", $this->resoures_path . \Library\Enums\FolderName::ResourceCommonFolderName . $file);
        }
        foreach ($this->_files_local as $file) {
            $this->loadFile("local", $this->resoures_path . \Library\Enums\FolderName::ResourceLocalFolderName . $file);
        }
    }
